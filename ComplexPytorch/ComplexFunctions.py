...
def complex_stageone_threshold(input, threshold):
    threshold = threshold.reshape(input.size(dim=1))
    threshold = threshold.repeat(input.size(dim=0))
    threshold = threshold.resize(input.size(dim=0), input.size(dim=1))
    f = MyFunctionStageOne.apply
    return f(input.real, threshold).type(torch.complex64) + 1j * relu(input.imag).type(torch.complex64)


class MyFunctionStageOne(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, threshold):
        x = torch.zeros((input.size(dim=0), input.size(dim=1)))
        ctx.save_for_backward(input, threshold)
        a = input.real - threshold.real
        # print(a)
        # print(x)
        results = torch.where(a >= 0, input.real, x)
        return results
    ...
    def backward(ctx, grad_output):
        ...
        grad_input[input.real < threshold.real] = 0
        y = torch.ones((input.size(dim=0), input.size(dim=1)))
        return grad_input, None


def complex_stagetwo_threshold(input):
    y = input.real.clone().detach()
    f = MyFunctionStageTwo.apply
    return relu(input.real).type(torch.complex64) + 1j * f(input.imag, y).type(torch.complex64)

class MyFunctionStageTwo(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input, y):
        x = torch.zeros((input.size(dim=0), input.size(dim=1)))
        ctx.save_for_backward(input, y)
        result = torch.where(y >= 0, input, x)
        return result

    @staticmethod
    def backward(ctx, grad_output):
        input, y, = ctx.saved_tensors
        grad_input = grad_output.clone()
        grad_input[y < 0] = 0
        y = torch.ones((input.size(dim=0), input.size(dim=1)))
        # print(grad_input)
...
def complex_recovery(input, wormsindex_tensor):
    # print('==============================================')
    a, b = input.shape
    wormsindex_tensor = wormsindex_tensor.resize(b).repeat(a).resize(a, b)
    # print(wormsindex_tensor.shape)
    # print(input.shape)
    f = MyFunctionStep.apply
    return f(input.real - wormsindex_tensor).type(torch.complex64) + 1j * relu(input.imag).type(torch.complex64)

class MyFunctionStep(torch.autograd.Function):
    @staticmethod
    def forward(ctx, input):
        ...
        output = torch.zeros(a, b, dtype=torch.float)
        for i in range(a):
            for j in range(b):
                if input[i, j].real.item() >= 0.:
                    output[i, j] = 1.
        return output
    ...
    def backward(ctx, grad_output):
        ...
        x = torch.ones((input.size(dim=0), input.size(dim=1)))
        # print(x - torch.tanh(input))
        # print(grad_output)
        return (x - torch.sigmoid(input)) * torch.sigmoid(input)


def complex_sigmoid(input):
    return sigmoid(input.real).type(torch.complex64) + 1j * input.imag.type(torch.complex64)
